...
//Hides all xm8 apps controls then deletes them for a smooth transition
...
//set XM8 title
(_display displayCtrl 4004) ctrlSetStructuredText (parseText (format ["<t align='center' font='RobotoMedium'>Air Drops</t>"]));
...
// Establish Category List from config file
  ...
  for "_i" from 0 to (count APOC_AA_Drops)-1 do {
    _Category = (APOC_AA_Drops select _i) select 0; //Grabs the string of the drop category ie: "Vehicles" or "Supplies"
    ...
    {
      {
        ...
        _Drop = format ["%1 - %2 respect", _DropDesc, _DropPrice];
...
fn_OrderDrop = {
  ...
  //Very convoluted system to extract the price from the arrays
    ...
    if (_DropPrice > _playerRespect) exitWith
      {
         format["You don't have enough respect to request this airdrop!"];
        _NotificationText =  format["You don't have enough respect to request this airdrop!", ceil _timeRemainingReuse];
        ["Whoops",[_NotificationText]] call ExileClient_gui_notification_event_addNotification;
        playSound "FD_CP_Not_Clear_F";
      };
    /////////////////////////
